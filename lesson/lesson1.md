...
- Реализуйте класс `ArrayStorageVersion` (вставте код в пустые методы класса): работа с `Resume` на основе массива (методы `clear, get, save, delete, getAll, size`)
...
### Optional
- Модифицировать класс `ArrayStorageVersion`: хранить все резюме в начале storage (без дырок null), чтобы не перебирать каждый раз все 10000 элементов.
```
